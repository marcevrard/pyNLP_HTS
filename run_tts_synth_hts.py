    ...
    try:
        while True:
            ...
            if prompt_utt == quit_com:
                ...
                idx = 0
                base_fpath = '{base_path}_{idx:04d}'.format(base_path=base_gen_fpath, idx=idx)
...
def process_recursive_synth(proc_path):
    ...
    for utt in process_utt_lst:
        ...
        if args.style != '':
            base_fpath = '{base_path}_{style}_{idx:04d}'.format(base_path=base_gen_fpath, style=args.style, idx=i_proc)
        else:
            base_fpath = '{base_path}_{idx:04d}'.format(base_path=base_gen_fpath, idx=i_proc)
